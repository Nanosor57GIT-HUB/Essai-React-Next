...
const FormBonneteau = ({ show, onClose }) => {
  ...
  return (
    <div
      className={`${styles.modalgamegagné} ${modalActive ? styles.active : ""}`}
    >
      <div className={styles.closecontainerformwinner}>
        <div className={styles.closewinner}> Fermer</div>
        <span className={styles.closeformwinner} onClick={onClose}>
          X
        </span>
      </div>
      ...
      <div className={styles.formcontainer}>
        {isSubmitted ? (
          <div className={styles.confirmationwinner}>
            <p className={styles.messageconfirmation}>
              Merci {formData.firstName} {formData.lastName} pour votre
              participation. Vous serez contacter dans un délai raisonnable à
              l&#39;adresse {formData.email} .
            </p>
            <button
              className={styles.btncloseconfirmationwinner}
              onClick={onClose}
            >
              Fermer
            </button>
          ...
          <form className={styles.forwinner} onSubmit={handleSubmit}>
            <h3 className={styles.bravotexte}>
              Bravo!{" "}
              <span className={styles.bravotextesuite}>Vous avez gagner</span>
            </h3>
            <div>
              <label htmlFor={styles.firstName}>
                Prénom :
                <input
                  type="text"
                  id={styles.firstName}
            ...
            <div className={styles.btnformwinner}>
              <button type="submit" className={styles.btnsubmitformwinner}>
                Soumettre
              </button>
            </div>
          </form>
